...
#define IO_LED_B_PIN GPIO_NUM_4
#define IO_LED_R_PIN GPIO_NUM_2
#define IO_ON_OPA_PIN GPIO_NUM_25

#define IO_SetLedRed() gpio_set_level(IO_LED_R_PIN, 1);
#define IO_ResetLedRed() gpio_set_level(IO_LED_R_PIN, 0);

#define IO_SetLedBlue() gpio_set_level(IO_LED_B_PIN, 1);
#define IO_ResetLedBlue() gpio_set_level(IO_LED_B_PIN, 0);

#define IO_EnableOPAPower() gpio_set_level(IO_ON_OPA_PIN, 0);
#define IO_DisableOPAPower() gpio_set_level(IO_ON_OPA_PIN, 1);
...
void IO_Config()
{
    ...
    //io_conf.pin_bit_mask = IO_ESPECIAL_PIN_SEL;  
    gpio_config(&io_conf);

    /* Set the GPIO as a push/pull output*/ 
    
    gpio_set_direction(IO_LED_R_PIN, GPIO_MODE_OUTPUT);
    gpio_set_direction(IO_LED_B_PIN, GPIO_MODE_OUTPUT);
    gpio_set_direction(IO_ON_OPA_PIN, GPIO_MODE_OUTPUT);

}

void IO_ConfigPin(gpio_num_t pin, gpio_mode_t mode, gpio_int_type_t interrupt_mode){
    gpio_config_t io_conf; 
    io_conf.intr_type = interrupt_mode; 
    io_conf.mode = mode;   
    io_conf.pin_bit_mask = 1ULL<<pin;  
    gpio_config(&io_conf);
}

void IO_TogglePin(gpio_num_t pin){
    uint32_t new_state = 1 - (GPIO.out >> pin & 0x1);
    gpio_set_level(pin, new_state);
}
...
static void echo_task(void *arg)
{
    ...
    while (1) {
        ...
        {
            data[len]=0;
            uart_write_bytes(UART_PORT_NUM, "APP_REGION\n", 10);
            if (len == 1)
            {
                
                esp_rewrite_ota_data(ESP_PARTITION_SUBTYPE_APP_OTA_0); 
                uart_write_bytes(UART_PORT_NUM, "OTA0\n", 5);
                esp_restart();
        
            }
            else if(len == 2)
            {
                esp_rewrite_ota_data(ESP_PARTITION_SUBTYPE_APP_OTA_1); 
                uart_write_bytes(UART_PORT_NUM, "OTA1\n", 5);
                esp_restart();
...
void app_main(void)
{
    ...
    xTaskCreate(blink_task, "led_blink_task", TASK_STACK_SIZE, NULL, 11, NULL);    
